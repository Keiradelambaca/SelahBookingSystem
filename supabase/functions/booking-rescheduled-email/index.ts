...
// supabase/functions/booking-rescheduled-email/index.ts
  ...
  try {
    ...
    const html = `
      <div style="font-family:Arial,sans-serif">
        ...
        <p>Your appointment with <b>${providerName}</b> has been rescheduled to:</p>
        <p><b>${startLocal}</b></p>
      </div>
    `;
    ...
    if (provider?.email) {
      ...
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: EMAIL_FROM,
          to: [provider.email],
          subject: `Rescheduled booking: ${startLocal}`,
          html: providerHtml,
        }),
      });
    }
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/booking-rescheduled-email' \
